!(function (e) {
					...
					},
					{
						...
						value: function () {
							for (
								...
								n[i] = arguments[i];
							if (!n.length)
								throw 'No input parameteres supplied to invalidate';
							n.forEach(function (t) {
								var n,
									i,
									o = {};
	...
	function (e, t, n) {
		...
		n.r(t),
			function (e) {
				n(4), n(5), n(6), n(7), n(9);
				e('.step-more').on('click', function () {
					e(this).closest('.step-item').addClass('open');
				}),
					e('.future-item-title').on('click', function () {
						e(this).closest('.future-item').toggleClass('open');
					}),
								...
								!(function () {
									fetch('https://ipapi.co/json/')
										...
										.then(data => {
											...
											const areaCodes = document.getElementsByClassName('area_code');
											const itiElements = document.getElementsByClassName('iti');

											for (let a = 0; a < phoneInputs.length; a++) {
												const u = intlTelInput(phoneInputs[a], { initialCountry: userCountry });
												itiElements[a].style.width = '100%';
												areaCodes[a].value = '+' + u.getSelectedCountryData().dialCode;

												phoneInputs[a].addEventListener('countrychange', function (t) {
													const n = window.intlTelInputGlobals.getInstance(this);
													e(this)
														.closest('form')
														.find('.area_code')
										...
										.catch(error => {
											console.error('Error while obtaining IP:', error);
										});
								})();
							})
							.catch(function () {
		...
		(function (e) {
			...
			function s(e) {
				...
				if ('hu' == t)
					switch (e) {
						case 'There was an error processing your request, please try again later.':
							e =
								'Hiba történt kérése feldolgozása során, próbálja meg később!';
							break;
						case 'You are already registered in our system.':
			...
			o.forEach(function (e, t) {
				...
				e('[data-id="form"]').submit(function (n) {
					n.preventDefault();
					var r = e(this),
						i = e(this).attr('data-lang')
							? '/' + e(this).attr('data-lang')
							: '',
						o = e(this).serializeArray();
					if (
						((t = e(this).attr('data-lang')),
						r.find('.alert').text('').addClass('hidden'),
						r.find('[name=agree]').length &&
							!r.find('[name=agree]').is(':checked'))
					)
						return (
							e('#openModalLoading').addClass('open'),
							setTimeout(function () {
								e('#openModalLoading').removeClass('open'),
									r.find('.alert').text('').removeClass('hidden'),
									r
										.find('.alert')
										.append(
					...
					a[u].validate(),
						a[u].updateValidationState(),
						a[u].updateValidationStatus(),
						a[u].validationStatus &&
							(e('#openModalLoading').addClass('open'),
							window.location.href.indexOf('stest') > -1 &&
								o.push({ name: 'stest', value: !0 }),
							o.push(
								{ name: 'funnel', value: e(this).attr('data-funnel') },
								{ name: '_ip', value: e(this).attr('data-ip') }
							),
							e.ajax({
								url: '/api/',
								type: 'POST',
								...
								error: function (t) {
									setTimeout(function () {
										e('#openModalLoading').removeClass('open'),
											r.find('.alert').text('').removeClass('hidden'),
											e.each(
												JSON.parse(t.responseText).errors,
												function (e, t) {
...
//========================================================================================================================================================
let links = document.querySelectorAll('.scroll-top-btn');
...
async function redirectToLanguage() {
    ...
    function setupLanguageSwitcher() {
        ...
        if (languageSelector) {
            ...
            languageSelector.addEventListener('change', (e) => {
                ...
                window.location.replace(newTargetUrl);
            });
        }
    }
    ...
    ) && !sessionStorage.getItem('redirectAttempted');
...
document.querySelector('form[data-id="form"]').addEventListener('submit', async (e) => {
    e.preventDefault();
...
📱 Телефон: ${phone}
    ...
    try {
        ...
        alert('Failed to send the application. Try again later.');
    }
});